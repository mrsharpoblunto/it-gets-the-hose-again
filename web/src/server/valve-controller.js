...
export default class ValveController extends EventEmitter {
  constructor(history, logger) {
    ...
    try {
      onoff = require('onoff');
      this.valveGpio = config.VALVE_GPIO
        ? new onoff.Gpio(config.VALVE_GPIO, 'out')
        : null;
      this.statusGpio = config.STATUS_GPIO
        ? new onoff.Gpio(config.STATUS_GPIO, 'out')
        : null;
    } catch (err) {
      this.valveGpio = null;
  ...
  async _setOpen(open, source) {
    ...
    this.logger.verbose(
      `Valve set by ${source} to ${open ? 'open' : 'closed'}`,
    );
    try {
      await this.history.write(
        source,
        open ? 'Watering started' : 'Watering stopped',
      );
    } catch (err) {
  ...
  getOpen() {
    this.logger.verbose(
      `Retrieved Valve state: ${this.open ? 'open' : 'closed'}`,
    );
    return this.open;
  }
}
