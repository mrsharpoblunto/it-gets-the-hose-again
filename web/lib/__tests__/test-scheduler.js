...
describe('Test scheduler', function () {
  it("Waters the garden as scheduled and that auto cutoff doesn't affect scheduled waterings", async done => {
    ...
    const testFetcher = jest.fn(() => Promise.resolve());
    ...
    valveController.on('setOpen', ({open, source}) => {
      ...
      if (testStep === 1) {
        expect(open).toBe(true);
        expect(storage[config.SCHEDULE_KEY].items[0].nextRun).toEqual(
          new Date(2016, 1, 25, 0, 0, 0).getTime() +
            2 * 86400 * 1000 +
            10 * 3600 * 1000,
        );
      }
      ...
      if (testStep === 5) {
        expect(storage[config.SCHEDULE_KEY].items[0].nextRun).toEqual(
          new Date(2016, 1, 27, 0, 0, 0).getTime() +
            2 * 86400 * 1000 +
            10 * 3600 * 1000,
        );
        done();
  ...
  it("Doesn't water if its already raining", async done => {
    ...
    const valveController = new NullValveController();
    const testFetcher = jest.fn().mockReturnValueOnce(
      Promise.resolve({
        json: function () {
          return Promise.resolve({
            weather: [
    ...
    valveController.on('setOpen', ({open}) => {
      ...
      expect(testFetcher).toHaveBeenCalledWith(
        'http://api.openweathermap.org/data/2.5/weather?lat=20&lon=60&appid=xxxx',
      );
      done();
    });
    await scheduler.start(true);
