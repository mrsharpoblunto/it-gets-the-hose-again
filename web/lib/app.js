...
function configureRoutes(app) {
  ...
  app.get('/socket.io*', (req, res) => {
    const message =
      'You are not running this application via webpack-dev-server. Browse to this application using the webpack-dev-server port to enable webpack support';
  ...
  server
    ...
    .on('error', err => {
      if (started) {
        ...
        app.logger.error(
          `Unable to listen on port ${config.APP_SERVER_PORT}. This is usually due to the process not having permissions to bind to this port. Did you mean to run the server in dev mode with a non-priviledged port instead?`,
        );
        process.exit(1);
      } else if (err.code === 'EADDRINUSE') {
        app.logger.error(
          `Unable to listen on port ${config.APP_SERVER_PORT} because another process is already listening on this port. Do you have another instance of the server already running?`,
        );
        process.exit(1);
      }
    });
